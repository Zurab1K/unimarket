...
function normalizeGeminiError(error: unknown): { status: number; message: string } {
  ...
  try {
    payload = JSON.parse(error.message) as GeminiErrorPayload;
  } catch {
    payload = null;
  }
  ...
  if (
    ...
    return {
      status: 429,
      message:
        "Gemini quota is unavailable for this project or has been exceeded." +
        retryHint +
...
function generateFallbackResponse(prompt: string, intent: string): string {
